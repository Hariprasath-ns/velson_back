...
import indexCreationRoute from "./routes/indexCreationRoute.js";
...
app.use("/api", indexCreationRoute);
...
const server = app.listen(PORT, async () => {
  ...
  setInterval(async () => {
    try {
      ...
      if (neonPrisma) {
        tasks.push(neonPrisma.tokenBlacklist.deleteMany({ where: { expiresAt: { lt: now } } }));
      }
